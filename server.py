        ...
        if random.random() < loss_prob:
            ...
            if checksum.client_server_checksum(data[3], data[1]) == 0:
                ...
                if data[0] == last_received_packet + 1:
                    a_socket.sendto(pickle.dumps([data[0] + 1, "0000000000000000", "1010101010101010"]), (ACK_HOST_NAME, 65021))
                    last_received_packet += 1
                    with open(file_name, 'ab') as file:
                        file.write(data[3])
                else:
                    a_socket.sendto(pickle.dumps([last_received_packet + 1, "0000000000000000", "1010101010101010"]),
                                    (ACK_HOST_NAME, 65021))
                a_socket.close()
            else:
